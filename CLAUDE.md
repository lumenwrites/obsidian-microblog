...
- **Committing & releasing**: **Every commit is a release.** Commit directly on `main` (no feature branches), and **bump the version on every commit** so the action publishes — before committing, run `npm version patch --no-git-tag-version` (use `minor`/`major` for bigger changes) to sync `package.json` + `manifest.json` + `versions.json`, then commit those together with your changes. Don't create git tags by hand — the action derives the tag from the manifest. **Don't push; the user pushes.** Pushing to `main` triggers `.github/workflows/release.yml`, which builds and publishes a release with the BRAT assets for that version (the repo is **public**, and the plugin ships to the user's iPad via **BRAT**). Before committing: `npm run build` + `npm run lint` must pass, and **never commit vault data, secrets, or `data.json`** (`main.js`/`node_modules`/`data.json` stay gitignored; CI rebuilds `main.js`). Full details in `docs/architecture.md` → "Distribution & releases".
