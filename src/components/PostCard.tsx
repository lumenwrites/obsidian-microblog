 ...
 *
 * `depth` is its reply nesting (0 = root); `depth > 0` marks it as a reply (drawing the
 * thread bar). `indent` is the visual indent level (flat for linear threads, per-depth
 * once a thread branches — see Timeline's walk). `isReplyTarget` highlights the target.
 */
