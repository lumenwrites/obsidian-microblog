...
`components/MarkdownPreview.tsx` renders a body via `MarkdownRenderer.render(app, md, el, sourcePath, component)` in an effect. Correctness: a per-render child `Component` is `load()`ed and `unload()`ed on cleanup (so embeds/post-processors unregister), `sourcePath` is the post's own path (so relative links/embeds/`[[wikilinks]]` resolve), and the container is `empty()`d before each render. **The container carries the `markdown-rendered` class** — `MarkdownRenderer.render` emits correct HTML (`<blockquote>`, callouts, tables, code blocks) but Obsidian's/the theme's reading-view styling is scoped under `.markdown-rendered`, so without that class those elements render unstyled (e.g. a blockquote shows as plain indented text with no left bar). Tags render once — inline, by Obsidian — and a capture-phase click listener on `a.tag` intercepts them (before Obsidian's global-search handler) to drive *our* search instead. This is the *output* side; the composer's *input* is a plain textarea (no live-preview editor — only undocumented internal APIs exist for that, and we don't need them since Edit opens the real note).
