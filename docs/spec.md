...
**Threads / Replies**
- Any post can reply to another (stored as a quoted `reply_to: "<parent-id>"` in frontmatter, where the id is the parent's filename stem). Replies nest under their parent in the timeline, each marked with a brighter-gray left thread-bar (and square left corners). **Indentation only steps in at branch points** — a linear chain (each post has a single reply) renders flat at the same indent, Twitter/Bluesky self-thread style; indentation advances one level only when a post forks into 2+ replies, so the offset shows structure only where the thread actually branches (capped at 5 levels). Enables play-by-post roleplaying threads (replying to your own posts).
