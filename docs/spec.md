...
**Threads / Replies**
- Any post can reply to another (stored as a quoted `reply_to: "<parent-id>"` in frontmatter, where the id is the parent's filename stem). Replies nest under their parent in the timeline, indented one level per reply depth (Reddit-style, capped at 5 levels so deep chains stay readable), each marked with a brighter-gray left thread-bar and square left corners. (We tried flattening linear self-chains Twitter/Bluesky-style, but it made a reply-to-a-reply look identical to a sibling reply — per-depth indentation is unambiguous.) Enables play-by-post roleplaying threads (replying to your own posts).
