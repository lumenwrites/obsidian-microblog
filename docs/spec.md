...
**Threads / Replies**
- Any post can reply to another (stored as a quoted `reply_to: "<parent-id>"` in frontmatter, where the id is the parent's filename stem). Replies nest under their parent in the timeline, each marked with a brighter-gray left thread-bar and square left corners. **Indentation adapts per thread:** while a thread is a single linear chain it renders *flat* (all replies at one indent, Twitter/Bluesky self-thread style) — unambiguous because a linear chain has no siblings; as soon as it branches *anywhere*, the whole thread switches to Reddit-style per-depth indentation (capped at 5 levels) so a reply-to-a-reply is never mistaken for a sibling. The branch/flat decision is made per root thread, so a thread is wholly one mode or the other (no mixing within a thread). Enables play-by-post roleplaying threads (replying to your own posts).
